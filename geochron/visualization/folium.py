...
def timehex_styledict(timehex: pd.DataFrame, opacity= float(.7), cmap:Optional[Callable] = None):
    ...
    .apply(lambda x: {'color': used_cmap(x), 'opacity': 0 if x == 0 else opacity}).to_dict()).to_dict()
...
def add_hashmap_properties(original_hashmap, time, cmap: Callable, opacity= float(.7)):
    import h3
    new_dict = {}
